...
$("#roomSaveBtn").on("click", function() {
	...
	$.ajax({
        ...
        <div style="position: relative">
         ...
         <p class="itemTab_roomDetailMore" data-no="${re.item_no}&${re.room_no
					}">더 보기</p>
        </div>
...
$("#roomMoreImgInput").on("change", function(e) {
	...
	arr.forEach((data) => {
		...
		reader.onload = function() {
			...
			$("#roomMoreImgCont div").append(img);
...
$("body").on("click", function(e) {
	...
	if ($(e.target).has(".fa-circle-exclamation").length || $(e.target).is('.fa-circle-exclamation')) {
		$(e.target).nextAll("div").eq(0).toggle();
	}
